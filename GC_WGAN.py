#########################################################################
# File Name: GC-SWGAN.py
# Author: Zhijian
# Created Time: 2025-02-07 Friday 05:20:01
#########################################################################
#!/usr/bin/env python3
import os
import time
...
MODEL_NAME = 'Galaxy10_GC_SWGAN_Train_test_91'
...
NCLASSES = 10 # number of class

#################
STEPs = 100000
#################
CURRENT_STEP = 0 # Epoch start from
SAVE_EVERY_N_STEP = 1000 # Save checkpoint at every n epoch
...
class Dataset:
    def __init__(self):
        ...
        train_idx, test_idx = train_test_split(np.arange(labels.shape[0]), test_size=0.1, random_state = 16)
        self.x_train, self.y_train, self.x_test, self.y_test =  images[train_idx], labels[train_idx], images[test_idx], labels[test_idx]

        self.num_labeled = self.y_train.shape[0] # Number of labeled galaxies
...
def SGAN_generator(input_z_shape=NOISE_DIM):
    ...
    #label_embedding = Flatten()(Embedding(NCLASSES,input_z_shape)(input_labels))
...
def SGAN_discriminator(input_x_shape=(IMG_WIDTH, IMG_HEIGHT, CHANNELS)):    #判别器的输入不卷入label
    '''
...
discriminator_unsup = SGAN_discriminator_unsup(discriminator)   #discriminator

def SGAN_discriminator_sup(discriminator,num_classes=NCLASSES):
    model = Sequential()
    model.add(discriminator)
    ###The following two layers can be omitted.
    ########################
    #model.add(Dense(128))
    #model.add(Dropout(0.15))
    #######################
...
discriminator_sup = SGAN_discriminator_sup(discriminator,num_classes=NCLASSES)  #classfier
...
def CWGAN_GP_train_d_sup_step(sup_image,label_oh,batch_size, step):
    ...
    '''
        One classfier (d_sup) training step
...
for step in range(CURRENT_STEP+1, STEPs + 1):   #!!!!!first +1 is template
    start = time.time()
...
#image_train,label_train = dataset.training_set()

print(image_test.shape)
#print(image_train.shape)

test_pred = discriminator_sup.predict(image_test)
#train_pred = discriminator_sup.predict(image_train)
